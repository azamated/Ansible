...
# Create VPC
resource "digitalocean_droplet" "vpc-Ansible-07" {
  name     = "vpc-Ansible-07"
  size     = "s-1vcpu-1gb"
...
# Get VPS data
data "digitalocean_droplet" "vpc-Ansible-07" {
  name = "vpc-Ansible-07"
  depends_on = [digitalocean_droplet.vpc-Ansible-07]
}

# Print public IP addresses
output "ip_address" {
  value = data.digitalocean_droplet.vpc-Ansible-07.ipv4_address
  }

# Save the valus of the public IP address on to a local variable
locals {
  public_ip = data.digitalocean_droplet.vpc-Ansible-07.ipv4_address
}
...
data "template_file" "vps_yaml_inventory" {
  ...
  vars = {
     ip_addr = data.digitalocean_droplet.vpc-Ansible-07.ipv4_address
  }
}
